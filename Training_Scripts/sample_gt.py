def sample_annotations_beat(annotations,annotation_sample_times,sample_times,numStates):
  ...
  sampled = [0]*(number_windows+1) # Store the annotations of a song
  t_anns = 1
  t_prev_anns = 1
  t_sample = 1
  ...
  while (t_sample <= number_windows and t_anns <= number_samples):
    ...
    if (sample_times[t_sample - 1] <= annotation_sample_times[1, t_anns - 1]):
      ...
      if (t_prev_anns < t_anns):
        ...
        # First interval
        if t_sample == 1:
          intervalC[countInterval - 1] = annotation_sample_times[1, t_prev_anns - 1] - annotation_sample_times[0, t_prev_anns - 1]
        else:
          intervalC[countInterval - 1] = annotation_sample_times[1, t_prev_anns - 1] - sample_times[t_sample - 2]
                
        countInterval=countInterval + 1
                
        # Between intervals
        for j in range(t_prev_anns+1,t_anns):                      
          intervalC[countInterval - 1] = annotation_sample_times[1, j - 1] - annotation_sample_times[0, j - 1]                        
          countInterval = countInterval + 1                   
                
        # Last interval
        intervalC[countInterval - 1] = sample_times[t_sample - 1] - annotation_sample_times[0, t_anns - 1]     
  ...
  if (t_anns == number_samples): # The last chord after final beats
    ...
    # Majority vote
    intervalC[countInterval - 1] = annotation_sample_times[1, t_anns-1] - sample_times[number_windows - 1]
    countInterval = countInterval + 1
